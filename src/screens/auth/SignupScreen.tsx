...
const SignupScreen: React.FC = () => {
  ...
  const handleSignup = async () => {
    if (!name || !email || !password || !confirmPassword) {
      showToast({ message: 'Meherbani karke sab information bhariye', type: 'error' });
      return;
    }
    if (password !== confirmPassword) {
      showToast({ message: 'Password match nahi ho raha', type: 'error' });
      return;
    }
    clearError();
    try {
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <SafeAreaView className="flex-1">
        ...
        >
          <ScrollView 
            contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 25, paddingTop: 50 }} 
          ...
          >
            ...
            {/* Glass Card */}
            <Animated.View entering={FadeInDown.delay(400).springify()} className="rounded-[35px] overflow-hidden bg-white/5 border border-white/15 mb-8">
